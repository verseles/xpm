...
pub async fn run() -> Result<()> {
    ...
    Logger::info(&format!("Checking {} installed packages...", installed.len()));
    ...
    for pkg in &installed {
        ...
        if installed_version != latest_version {
            upgradable.push((pkg.clone(), installed_version.clone(), latest_version.clone()));
        }
    }
    ...
    Logger::info(&format!("{} packages can be upgraded:", upgradable.len().to_string().yellow()));
