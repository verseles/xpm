...
void main() {
  group('HomebrewPackageManager', () {
    late HomebrewPackageManager manager;

    setUp(() {
      manager = HomebrewPackageManager();
    });
    ...
    test('should search for packages', () async {
      try {
        ...
        // Skip test if brew is not available
    ...
    test('should check if package is installed', () async {
      try {
        final isInstalled = await manager.isInstalled('git');
        // Git might or might not be installed via brew
        expect(isInstalled, anyOf(isTrue, isFalse));
      } catch (e) {
        expect(e, isA<Exception>());
      }
    }, tags: ['skip-ci']);
